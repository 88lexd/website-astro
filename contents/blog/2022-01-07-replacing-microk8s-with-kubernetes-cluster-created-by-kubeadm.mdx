...
As I already have my existing Terraform code to create the VPC etc. I only needed to create a sub-module to provision these new additional resources. The source code for Terraform can be found here on my [GitHub page](https://github.com/88lexd/lexd-solutions/tree/main/aws-wordpress/1-infra-configuration/2-general-setup). If you want to know more about my existing Terraform code, see my [previous post](https://lexdsolutions.com/2021/09/using-cloudformation-and-terraform-on-my-brand-new-aws-account/).

The new Terraform module will provision and configure the following additional resources on AWS:

**EC2 Instances** ([ec2.tf](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/1-infra-configuration/2-general-setup/kubeadm_cluster/ec2.tf))
...
**Security Groups (**[security-groups-tf](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/1-infra-configuration/2-general-setup/kubeadm_cluster/security-groups.tf))
...
**EC2 IAM Role** ([ec2-iam-role.tf](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/1-infra-configuration/2-general-setup/kubeadm_cluster/ec2-iam-role.tf))
...
**EFS (Elastic File System)** ([efs.tf](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/1-infra-configuration/2-general-setup/kubeadm_cluster/efs.tf))
...
**Terraform Variables** ([variables.tf](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/1-infra-configuration/2-general-setup/kubeadm_cluster/variables.tf))
...
When the parent module ([kubeadm-cluster.tf](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/1-infra-configuration/2-general-setup/kubeadm-cluster.tf)) is triggered, it will pass in these additional values.
...
Same as my previous setup. Everything within the OS is configured through Ansible. The source code can be found on my [GitHub page](https://github.com/88lexd/lexd-solutions/tree/main/aws-wordpress/2-os-configuration).

The Ansible playbook will configure the following:

- Apply base OS configuration with my personal settings on all nodes ([base-config.yml](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/2-os-configuration/ansible_tasks/base-config.yml))
- Install and Setup CloudWatch agent on all nodes ([setup-aws-cw-agent.yml](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/2-os-configuration/ansible_tasks/setup-aws-cw-agent.yml))
- Install and configure containerd as container runtime on all nodes ([setup-containerd.yml](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/2-os-configuration/ansible_tasks/setup-containerd.yml))
- Prepare nodes for Kubernetes install (e.g. disabling swap, configuring br\_netfilter etc.) ([setup-k8s-nodes.yml](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/2-os-configuration/ansible_tasks/setup-k8s-nodes.yml))
- Install Kubernetes packages on all nodes (e.g. kubeadm, kubectl, kubelet) ([setup-k8s-nodes.yml](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/2-os-configuration/ansible_tasks/setup-k8s-nodes.yml))
- Create Kubernetes cluster by using kubeadm ([k8s-master.yml](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/2-os-configuration/ansible_tasks/k8s-master.yml))
- Join worker node to cluster ([k8s-join-nodes-to-cluster.yml](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/2-os-configuration/ansible_tasks/k8s-join-nodes-to-cluster.yml))
- Apply cluster config such as deploying existing secrets and creating namespaces ([k8s-cluster-configs.yml](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/2-os-configuration/ansible_tasks/k8s-cluster-configs.yml))
- Install and setup Nginx Ingress Controller ([setup-nginx-ingress-controller.yml](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/2-os-configuration/ansible_tasks/setup-nginx-ingress-controller.yml))
- Install and setup NFS-Subdir External Provisioner ([setup-nfs-subdir-provisioner.yml](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/2-os-configuration/ansible_tasks/setup-nfs-subdir-provisioner.yml))

**Ansible Inventory** ([inventory\_aws.ini](https://github.com/88lexd/lexd-solutions/blob/main/aws-wordpress/2-os-configuration/inventory_aws.ini))
